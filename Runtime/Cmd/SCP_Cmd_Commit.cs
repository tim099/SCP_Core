...
namespace SCP.Core.Cmd
{
    public sealed class SCP_Cmd_Commit : SCP_Cmd
    {
        ...
        public override string Details =>
            ...
            + "\n"
            + "🛡 `expect_files` **必填** —— 宣告這一筆該收幾個檔，不符就擋下（**在 commit 之前**返回）。\n"
            + "   它把「我以為我在提交幾個檔」變成一個必須先算過的數字。\n"
            + "   ⛔ 真的不想數就顯式打 `expect_files=any`：會**大聲印出整份 staged 清單**並落讀數。\n"
            + "   📌 放棄本身沒有錯，錯的是**放棄得沒有痕跡** ——\n"
            + "     「忘了帶」與「我想過了，這次不數」在它還是選填的時候是同一個畫面。\n"
            + "\n"
