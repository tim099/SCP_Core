...
namespace SCP.Core.Gui
{
    ...
    public abstract class SCP_GuiToolPage : SCP_GuiPage
    {
        ...
        ///
        /// <para>🔴 **什麼時候該回 null**（Tim 2026-09-22 點名的幾類，⛔ 這幾類都不要另外發明機制）：
        /// <list type="bullet">
        /// <item>**彈窗／選項頁** —— 內容由母頁決定（選項清單、標的物）
        /// ⇒ 憑一個 key 從外面把它叫起來只會得到一個沒有受詞的空頁，所以不列。</item>
        /// <item>**入口頁自己** —— 不然清單上會有一顆「開啟入口頁」的鈕，按下去是同一頁疊自己。</item>
        /// <item>**只從工具列進去的頁**（例：介面尺寸）—— 它有入口，只是不在這份清單上。</item>
        /// </list>
        /// ⚠ 而**給人繼承的基底頁不走這裡**：宣告成 <c>abstract</c> 就好 ——
        /// 那是語言自帶的記號、編譯期就擋掉 <c>new</c>，而頁面掃描本來就跳過抽象型別
        /// （2026-09-22 實測：抽象基底一筆缺陷都不會產生）。</para>
        ///
        /// <para>🔴 **界線：null 與 <see cref="SCP_PageIgnoreAttribute"/> 不是同一件事**，
        /// 而選錯是有代價的（不是風格問題）：
        /// <list type="bullet">
        /// <item><c>MenuGroup = null</c> ＝ **是頁，只是不列** ⇒ **照樣收錄**，
        /// <c>Create(key)</c> 造得出來、<c>--page</c> 也進得去。</item>
        /// <item><c>[SCP_PageIgnore]</c> ＝ **根本不是頁**（測試探針）⇒ **不收錄**，
        /// <c>Create</c> 回 null。</item>
        /// </list>
        /// 🩸 把彈窗頁貼成 <c>[SCP_PageIgnore]</c> 的後果（2026-09-22 實測）：
        /// <c>SCP_GuiPageController.PathKeys</c> 存的是**每一層的 Key**（含彈窗那層），
        /// 而 CLI 每次呼叫都是新 process ⇒ 下一次呼叫走 <c>RestorePath</c>，
        /// 它對每個 key 叫一次 <c>Create</c>；回 null 就照 D13 的規矩**停在那一層並回報**
        /// ⇒ 症狀是「在彈窗裡按第二下時，人其實已經掉回母頁了」。
        /// ⚠ 而「彈窗選項頁按下去」就是第二步 ⇒ 那不是邊角，是主路。
        /// 📌 射程：**只有 CLI／文字模式會**；視窗模式的導覽狀態活在記憶體裡，不走這條。</para>
        /// </summary>
