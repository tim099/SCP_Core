...
namespace SCP.Core.Compile
{
    ...
    public static class SCP_UnityCompile
    {
        ...
        //     的東西會製造永遠不會消失的假陽性。
        //   · 🩸 **已知假陽性（2026-09-10 驗收當場量到）：只改 mtime 而內容沒變**（`touch`）。
        //     Unity 用內容判斷要不要重編，本尺用 mtime ⇒ 兩把尺不同源，而那正是它的價值也是它的限制：
        //     `touch` 之後本尺報 `stale=1`，而組件其實仍然涵蓋那個檔的**內容**。
        //     ⇒ 判準寫清楚：本欄位答的是「**組件的時間戳有沒有落後原始碼**」，
        //     ⛔ 不是「組件的內容有沒有落後原始碼」。真實存檔會同時改內容與 mtime，所以日常有效。
